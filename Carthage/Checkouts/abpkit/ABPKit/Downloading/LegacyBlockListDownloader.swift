...
class LegacyBlockListDownloader: NSObject,
    ...
    var downloadEvents = [DownloadTaskID: BehaviorSubject<LegacyDownloadEvent>]()
    ...
    func blockListDownload(for filterList: LegacyFilterList,
                           runInBackground: Bool = true) -> Observable<URLSessionDownloadTask> {
        guard let urlString = filterList.source,
              let url = URL(string: urlString),
              var components = URLComponents(string: url.absoluteString)
        else { return Observable.error(ABPDownloadTaskError.failedToMakeDownloadTask) }
    ...
    func lastDownloadEvent(taskID: Int) -> LegacyDownloadEvent? {
