        ...
        let removersFor: (User, [String]) -> CompoundRemovers = { user, names in
            self.ruleListIdentifiers()
                .flatMap { ids -> CompoundRemovers in
                    ...
                    let publishers: [SingleRuleListID]? = ids?.filter { idr in !names.contains { $0 == idr } }
                        .map { self.listRemovedFromStore(identifier: $0) }
                        .reduce([]) { $0 + [$1] }
                    if let pubs = publishers, pubs.count > 0 {
                        return Publishers.Sequence(sequence: pubs).eraseToAnyPublisher()
                    }
                    // Empty string sent to keep operation chains continuous:
                    return Publishers.Sequence(sequence: [SinglePublisher("").eraseToAnyPublisher()]).eraseToAnyPublisher()
                }.eraseToAnyPublisher()
        }
        return { user in
            var all: [String]!
